...
const LINKS = [
  { label: "Marketplace", href: "/app/marketplace", icon: Storefront, desc: "Browse listings & live offers" },
      ...
      <div className="relative mx-auto max-w-[1240px] px-5 lg:px-10 flex items-center justify-between h-[68px]">
        ...
        <button type="button" onClick={() => setOpen(o => !o)} aria-label="Toggle navigation"
          ...
          <span className="relative block w-5 h-4" aria-hidden>
            <m.span className="absolute left-0 right-0 top-1/2 block h-[2px] -mt-px rounded-full bg-current" animate={open ? { y: 0, rotate: 45 } : { y: -6, rotate: 0 }} transition={{ duration: 0.3, ease: EASE }} />
            <m.span className="absolute left-0 right-0 top-1/2 block h-[2px] -mt-px rounded-full bg-current" animate={open ? { opacity: 0, scaleX: 0.2 } : { opacity: 1, scaleX: 1 }} transition={{ duration: 0.2, ease: EASE }} />
            <m.span className="absolute left-0 right-0 top-1/2 block h-[2px] -mt-px rounded-full bg-current" animate={open ? { y: 0, rotate: -45 } : { y: 6, rotate: 0 }} transition={{ duration: 0.3, ease: EASE }} />
          </span>
        </button>
      </div>
      <AnimatePresence>
        {open && (
          <>
            ...
            <m.div key="panel" initial={{ y: -12, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -12, opacity: 0 }} transition={{ duration: 0.35, ease: EASE }}
                ...
                {LINKS.map((l) => {
                  ...
                  return (
                    <Link key={l.label} href={l.href} onClick={() => setOpen(false)} className="group flex items-center gap-3.5 px-3 py-3 rounded-2xl hover:bg-bn-page-2 transition-colors">
                      <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-bn-page-2 text-bn-ink-2 group-hover:bg-bn-accent-10 group-hover:text-bn-accent transition-colors"><Icon size={18} weight="bold" /></span>
