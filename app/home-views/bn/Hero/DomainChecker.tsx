"use client";
import { ArrowRight, CircleNotch, CurrencyBtc, WarningCircle } from "@phosphor-icons/react/dist/ssr";
...
type CheckState = { kind: "idle" } | { kind: "invalid"; reason: string } | { kind: "checking" } | { kind: "available" } | { kind: "taken"; marketplaceContractId?: string };
...
export const DomainChecker = forwardRef<DomainCheckerHandle>((_p, ref) => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    // Save domain intent, redirect to login → login redirects to /app/buy/[name]
    if (state.kind === "available" || state.kind === "idle") {
      if (clean) {
        if (typeof sessionStorage !== "undefined")
          sessionStorage.setItem("post_login_redirect", `/app/buy/${clean}.btc`);
        router.push("/login");
      }
    } else if (state.kind === "taken") {
      if (typeof sessionStorage !== "undefined")
        sessionStorage.setItem("post_login_redirect", `/app/marketplace`);
      router.push("/login");
    }
  };
  ...
  const wrapClass = cn(
    ...
    isAvailable ? "border-bn-accent ring-4 ring-bn-accent/15"
      : isError ? "border-[rgba(204,68,102,0.6)]"
  ...
  return (
    <form onSubmit={onSubmit} className="w-full" aria-label="Check a .btc name">
      <div className={wrapClass}>
        <span className="hidden sm:flex items-center pl-5 text-bn-accent"><CurrencyBtc weight="bold" size={20} /></span>
        <div className="relative flex-1 flex items-center min-w-0">
          <input ref={inputRef} type="text" inputMode="text" autoComplete="off" spellCheck={false}
        ...
        <button type="submit" disabled={state.kind === "checking"}
          ...
          {state.kind === "checking"
            ...
            : <>{isAvailable ? "Buy" : "Check"} <ArrowRight weight="bold" size={14} /></>}
        </button>
      </div>
      <Status state={state} fullName={clean ? `${clean}.btc` : ""} />
    </form>
  );
});
...
const Status = ({ state, fullName }: { state: CheckState; fullName: string }) => {
  if (state.kind === "idle") return <p className="mt-3 text-[13px] text-bn-ink-muted min-h-[20px] pl-1">3–63 characters · letters, numbers, hyphens</p>;
  if (state.kind === "invalid") return <p className="mt-3 text-[13px] text-bn-danger flex items-center gap-2 min-h-[20px] pl-1"><WarningCircle weight="fill" size={14} />{state.reason}</p>;
  if (state.kind === "checking") return <p className="mt-3 text-[13px] text-bn-ink-muted flex items-center gap-2 min-h-[20px] pl-1"><span className="w-1.5 h-1.5 rounded-full bg-bn-ink-muted animate-pulse" />Checking on Bitcoin...</p>;
  if (state.kind === "available") return (
    <div className="mt-3 flex items-center justify-between gap-3 min-h-[20px] pl-1">
      <span className="text-[13px] text-bn-ink flex items-center gap-2">
        ...
        <span className="text-bn-ink-muted">is available — click Buy to continue</span>
      </span>
    </div>
  );
  const href = state.marketplaceContractId ? `/app/marketplace/${state.marketplaceContractId}` : null;
  return (
    <div className="mt-3 flex items-center justify-between gap-3 min-h-[20px] pl-1">
      ...
      {href && <a href={href} className="text-[13px] font-medium text-bn-ink hover:text-bn-accent inline-flex items-center gap-1">View on marketplace <ArrowRight weight="bold" size={12} /></a>}
    </div>
  );
};
