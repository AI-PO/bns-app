...
export const DomainChecker = forwardRef<DomainCheckerHandle>((_props, ref) => {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    // Save intended domain and redirect to login
    // Login will redirect back to /app/buy/[name] after auth
...
const Status = ({ state, fullName }: { state: CheckState; fullName: string }) => {
  ...
  if (state.kind === "available") return (
    <div className="mt-3 flex items-center justify-between gap-3 min-h-[20px] pl-1">
      ...
      <button type="submit" className="text-[13px] font-medium text-bn-accent hover:text-bn-accent-hover inline-flex items-center gap-1">
  ...
  return (
    <div className="mt-3 flex items-center justify-between gap-3 min-h-[20px] pl-1">
      ...
      <button type="submit" className="text-[13px] font-medium text-bn-ink hover:text-bn-accent inline-flex items-center gap-1">
