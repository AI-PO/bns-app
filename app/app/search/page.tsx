...
function SearchContent() {
  ...
  const FIELDS = [
    ...
    { icon: '🌐', label: 'Site', key: 'site', ph: 'ipfs://...' },
    { icon: '𝕏', label: 'Twitter', key: 'twitter', ph: '@handle' },
  ]

  return (
    <div className="p-7 max-w-[640px]">
      <h1 className="text-[26px] font-medium tracking-[-0.04em] text-bn-ink text-center mb-6">
      ...
      {status === 'checking' && <p className="font-mono-bn text-[13px] text-bn-ink-muted mb-4">Checking on Bitcoin...</p>}
      ...
      {status === 'taken' && (
        <div className="bg-white border border-bn-line-2 rounded-2xl p-4 flex items-center justify-between mb-4">
          ...
          <div className="flex items-center mb-7">
            {([1,2,3] as const).map((s, i) => (
              <div key={s} className="flex items-center flex-1">
                ...
                <span className={`ml-2 text-[12px] whitespace-nowrap ${step === s ? 'text-bn-ink' : 'text-bn-ink-muted'}`}>{['Confirm','Set Records','Complete'][s-1]}</span>
          ...
          {step === 2 && (
            <div>
              <div className="bg-white border border-bn-line rounded-2xl p-5 mb-3 shadow-[0_1px_3px_rgba(10,10,10,0.05)]">
                ...
                <p className="text-[14px] text-bn-ink-muted">Map your name. Update any time.</p>
              </div>
              <div className="bg-white border border-bn-line rounded-2xl overflow-hidden mb-4 shadow-[0_1px_3px_rgba(10,10,10,0.05)]">
                {FIELDS.map(f => (
                  <div key={f.key} className="flex items-center border-b border-bn-line last:border-0">
